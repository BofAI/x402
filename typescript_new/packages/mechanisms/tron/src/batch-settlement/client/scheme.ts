...
import {
  ...
  type BatchSettlementTronSchemeOptions,
...
/**
 * Client-side implementation of the `batch-settlement` scheme for TRON networks.
 *
 * Builds payment payloads (deposit + voucher, or voucher-only), reconciles server
 * responses, handles corrective 402 resynchronisation, and supports cooperative
 * refunds.
 */
export class BatchSettlementTronScheme implements SchemeNetworkClient {
  ...
  private readonly payerAuthorizer: string | undefined;
  private readonly voucherSigner: ClientTronSigner | undefined;

  /**
   * Constructs a batched client scheme.
   *
   * @param signer - Client TRON wallet used for signing vouchers and deposit authorizations.
   * @param optionsOrPolicy - Either a full options object or a bare deposit-policy.
   */
  constructor(
    private readonly signer: ClientTronSigner,
    optionsOrPolicy?: BatchSettlementTronSchemeOptions | BatchSettlementDepositPolicy,
  ) {
    ...
    if (
      ...
      normalizeAddressForSigning(payerAuthorizer) !==
        normalizeAddressForSigning(voucherSigner.address)
    ) {
  ...
  /**
   * Creates the payment payload for a batched request (deposit+voucher or voucher-only).
   *
   * @param x402Version - Protocol version for the payload envelope.
   * @param paymentRequirements - Server payment requirements.
   * @param context - Optional payment payload context (unused).
  ...
  async createPaymentPayload(
    ...
    if (needsInitialDeposit || needsTopUp) {
      ...
      if (assetTransferMethod === "permit2") {
        return createBatchSettlementPermit2DepositPayload(
          this.signer,
          x402Version,
          paymentRequirements,
          config,
  ...
  private normalizeStrategyDepositAmount(value: string | bigint): string {
    ...
    if (/^\d+$/.test(value) && BigInt(value) > 0n) return BigInt(value).toString();
    throw new Error("depositStrategy must return a positive integer deposit amount");
  }
