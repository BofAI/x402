 ...
 * @param url - Any protected route on the channel to refund.
...
async function executeRefund(
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
    ...
    if (response.status === 402) {
      ...
      if (nonRecoverable) throw new Error(nonRecoverable);
    }
    ...
    if (response.status === 402) {
      if (result.recovered && attempt < maxAttempts) continue;
      if (result.recovered) {
 ...
 *
 * @param ctx - Identity inputs (storage, signers, salt, payerAuthorizer).
 * @param paymentRequired - Decoded 402 body from the probe.
...
async function buildRefundPaymentPayload(
  ...
  if (channel === undefined) {
    throw new Error("Refund requires an existing channel record; deposit first");
  }
...
/**
 * Creates an x402 HTTP client for batch settlement with hooks; refund payloads are
 * supplied by {@link refundChannel} instead of the default payment builder.
 *
 * @param ctx - Identity inputs (storage, signers, salt, payerAuthorizer).
...
/**
 * If the refund HTTP response cannot be recovered by retrying, returns a
 * user-facing message; otherwise returns `undefined`.
 *
 * @param response - The refund request response.
...
function formatRefundFailure(settle: SettleResponse): string {
  ...
  if (message && message !== reason) return `Refund failed: ${reason}: ${message}`;
