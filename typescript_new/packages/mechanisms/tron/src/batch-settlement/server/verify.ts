...
/**
 * Verifies the TIP-712 voucher signature against the payer authorizer.
 *
 * @param raw - Decoded batch-settlement voucher payload.
 * @param network - CAIP-2 network identifier for chain ID / domain.
...
async function verifyLocalVoucherSignature(
  ...
  try {
    const recovered = recoverTypedDataAddress(
      getBatchSettlementTip712Domain(network),
      voucherTypes,
      {
        channelId: raw.voucher.channelId,
    ...
    return (
      ...
      normalizeAddressForSigning(raw.channelConfig.payerAuthorizer).toLowerCase()
    );
  } catch {
    return false;
  }
 ...
 */
function invalidVerifyResponse(payer: string, invalidReason: string): VerifyResponse {
