...
| `expirationTime` | `string` | Optional | ISO 8601 timestamp when challenge expires. Emitted only when `expirationSeconds` is configured. |
...
- **Domain**: `domain` MUST match the hostname exposed by the HTTP adapter's current request URL.
- **URI**: `uri` MUST start with the origin exposed by the HTTP adapter's current request URL.
...
- **Nonce**: The server generates a fresh 16-byte nonce. Replay checks run when storage implements both nonce methods.

The SDK reports human-readable validation errors through the optional `onEvent` callback. A failed
SIWX attempt does not itself fail the request; the normal payment flow continues.
...
The SDK reports human-readable errors for unsupported namespaces, invalid encodings, signature
length errors, verifier failures, and invalid signatures.
...
- **Domain Binding**: The current hook validates against the hostname and origin exposed by the HTTP adapter. Behind a proxy, the adapter MUST expose the canonical browser-visible URL and must not trust unvalidated forwarded headers.
- **Nonce Uniqueness**: Challenges use fresh random nonces. Production storage SHOULD implement both `hasUsedNonce` and `recordNonce`; configuring only one is rejected.
