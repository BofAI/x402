"""TRON TIP-712 (TransferWithAuthorization) facilitator logic."""
...
def verify_eip3009(
    ...
    try:
        payer_for_contract = normalize_address_for_contract_call(str(auth.get("from", "")))
...
def settle_eip3009(
    ...
    r = bytes.fromhex(clean_sig[:64])
    s = bytes.fromhex(clean_sig[64:128])
    v = int(clean_sig[128:130], 16)
    nonce_bytes = bytes.fromhex(str(auth.get("nonce", "0x" + "00" * 32)).removeprefix("0x"))

    try:
        from_for_contract = normalize_address_for_contract_call(str(auth.get("from", "")))
        to_for_contract = normalize_address_for_contract_call(str(auth.get("to", "")))
