...
describe("resolveTronRpcUrl", () => {
  it("uses an explicit rpcUrl override above everything else", () => {
    expect(resolveTronRpcUrl("tron:nile", { rpcUrl: "https://my.host", apiKey: "k" })).toBe(
      "https://my.host",
    );
  });

  it("uses the keyed TronGrid default when an API key is supplied", () => {
    expect(resolveTronRpcUrl("tron:nile", { apiKey: "k" })).toBe("https://nile.trongrid.io");
    expect(resolveTronRpcUrl("tron:mainnet", { apiKey: "k" })).toBe("https://api.trongrid.io");
  });

  it("uses the key-less fallback for nile when no API key is set", () => {
    // nile.trongrid.io rate-limits unkeyed; nileex works key-less.
    expect(resolveTronRpcUrl("tron:nile")).toBe("https://api.nileex.io");
  });

  it("uses the key-less fallback for mainnet/shasta when no API key is set", () => {
    expect(resolveTronRpcUrl("tron:mainnet")).toBe("https://hptg.bankofai.io");
    expect(resolveTronRpcUrl("tron:shasta")).toBe("https://api.shasta.trongrid.io");
  });
