...
export function decodeSignedTrc20Approval(signedTransaction: string): DecodedTrc20Approval {
  ...
  const rawData = bytesField(outer, 1, "raw_data");
  const signature = bytesField(outer, 2, "signature");
  ...
  const expiration = uintField(raw, 8, "expiration");
  const contractBytes = bytesField(raw, 11, "contract");
  const timestamp = uintField(raw, 14, "timestamp");
  const feeLimit = uintField(raw, 18, "fee_limit");
  ...
  const owner = addressFromBytes(bytesField(trigger, 1, "owner_address"), "owner_address");
  const asset = addressFromBytes(bytesField(trigger, 2, "contract_address"), "contract_address");
  ...
  const spender = TronWeb.address.fromHex(`41${bytesToHex(calldata.slice(16, 36))}`);
  const amountBytes = calldata.slice(36, 68);
  if (amountBytes.some(byte => byte !== 0xff)) throw new Error("Approval amount is not MaxUint256");
...
export function validateTrc20ApprovalForPayment(
  ...
  try {
    ...
    if (
      normalizeTronAddress(info.asset) !== normalizeTronAddress(approval.asset) ||
      normalizeTronAddress(requirements.asset) !== normalizeTronAddress(approval.asset)
    ) {
    ...
    if (
      ...
      !options.allowedAssets.some(
        asset => normalizeTronAddress(asset) === normalizeTronAddress(approval.asset),
      )
    ) {
