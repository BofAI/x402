...
vi.mock("@bankofai/x402-legacy/client", () => ({
...
describe("withPaymentInterceptor()", () => {
  ...
  beforeEach(async () => {
    ...
    const { selectPaymentRequirements } = await import("@bankofai/x402-legacy/client");
  ...
  it("should reject if payment header creation fails", async () => {
    ...
    const { createPaymentHeader } = await import("@bankofai/x402-legacy/client");
...
describe("withPaymentInterceptor() - SVM and MultiNetwork", () => {
  ...
  it("passes [solana, solana-devnet] for SVM-only signers", async () => {
    vi.doMock("@bankofai/x402-legacy/types", async () => {
      const actual = await vi.importActual("@bankofai/x402-legacy/types");
      return {
        ...actual,
    ...
    vi.doMock("@bankofai/x402-legacy/client", () => ({
