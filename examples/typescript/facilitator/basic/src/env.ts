 ...
 * private key. The provider signs through a configured raw-secret, wallet-cli,
 * or Privy backend. A chain registers only when a
...
/**
 * Resolves the agent-wallet for an exact CAIP-2 network. agent-wallet 3.x
 * validates the requested network on every operation, so callers must not reuse
 * a wallet resolved for another network in the same family.
 */
/**
