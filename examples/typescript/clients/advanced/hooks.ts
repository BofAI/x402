...
import { x402Client } from "@bankofai/x402-fetch";
...
export async function runHooksExample(evmPrivateKey: `0x${string}`, url: string): Promise<void> {
  ...
  const { wrapFetchWithPayment } = await import("@bankofai/x402-fetch");
  ...
  const { x402HTTPClient } = await import("@bankofai/x402-fetch");
