...
const RESOURCE_URL =
  process.env.RESOURCE_URL || "http://localhost:4041/weather";
...
for (const t of targets) {
  ...
  if (REFUND_AFTER) {
    const scheme = refundable.find((s) =>
      usedNetwork ? matchesNetwork(s.label, usedNetwork) : false,
    );
    if (!scheme) {
      ...
      const result = await scheme.refund(
        RESOURCE_URL,
